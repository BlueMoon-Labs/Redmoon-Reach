...
/datum/controller/subsystem/communications/proc/make_announcement(mob/living/user, is_silicon, input)
	if(is_silicon)
		if(user.job)
			...
			priority_announce(html_decode(user.treat_message(input)), "[used_title] Указывает", pick('sound/misc/royal_decree.ogg', 'sound/misc/royal_decree2.ogg'), sender = user)
		...
		if(user.job)
			...
			if(SSticker.regentmob == user)
				used_title = "[used_title]" + " Regent"
			priority_announce(html_decode(user.treat_message(input)), "[used_title] Вещает", 'sound/misc/bell.ogg', sender = user)
		else
			priority_announce(html_decode(user.treat_message(input)), "Кто-то Вещает", 'sound/misc/bell.ogg', sender = user)
